...
size_t TextAnalysis::countTwoWords(const std::string& word1, const std::string& word2)
{
	...
	if (wordtable.find(word1) != wordtable.end())
	{
	    auto it1 = wordtable.find(word1);
	    auto it2 = wordtable.find(word2);
	    for( auto i: it1->second)
        {
            if (std::find(it2->second.begin(),it2->second.end(),i) != it2->second.end() && std::find(temp.begin(),temp.end(),i) == temp.end())
            {
                temp.push_back(i);
            }
        }
